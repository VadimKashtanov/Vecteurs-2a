...
void mdl_optimisation_DECODEUR(Mdl_t * mdl) {
	uint I = mdl->insts;
	//
	uint grille[I*I];
	uint     nb[I];
	...
	//
	mdl->BLOQUES_DECODEUR = premier_nulle-1+1;
	//
	mdl->elements_DECODEUR = (uint*)malloc(sizeof(uint) * mdl->BLOQUES_DECODEUR);
	FOR(0, i, mdl->BLOQUES_DECODEUR)
		mdl->elements_DECODEUR[i] = nb[i];
	//
	mdl->instructions_DECODEUR = (uint**)malloc(sizeof(uint*) * mdl->BLOQUES_DECODEUR);
	FOR(0, i, mdl->BLOQUES_DECODEUR) {
		mdl->instructions_DECODEUR[i] = (uint*)malloc(sizeof(uint) * nb[i]);
		FOR(0, j, nb[i])
			mdl->instructions_DECODEUR[i][j] = grille[i*I + j];
	}
};