...
void plumer_model  (Mdl_t * mdl) {
	...
	printf(" -- optimisation ENCODEUR --\n");
	FOR(0, i, mdl->BLOQUES_ENCODEUR) {
		printf("b=%i| ", i);
		FOR(0, j, mdl->elements_ENCODEUR[i]) printf("%i ", mdl->instructions_ENCODEUR[i][j]);
		printf("\n");
	};

	printf(" -- optimisation DECODEUR --\n");
	FOR(0, i, mdl->BLOQUES_DECODEUR) {
		printf("b=%i| ", i);
		FOR(0, j, mdl->elements_DECODEUR[i]) printf("%i ", mdl->instructions_DECODEUR[i][j]);
		printf("\n");
	};
};
