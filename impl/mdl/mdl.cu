...
void liberer_mdl(Mdl_t * mdl) {
	...
	free(mdl->inst);
	//
	FOR(0, i, mdl->BLOQUES_ENCODEUR) free(mdl->instructions_ENCODEUR[i]);
	free(mdl->instructions_ENCODEUR);
	free(mdl->elements_ENCODEUR);
	//
	FOR(0, i, mdl->BLOQUES_DECODEUR) free(mdl->instructions_DECODEUR[i]);
	free(mdl->instructions_DECODEUR);
	free(mdl->elements_DECODEUR);
	//
	free(mdl);
};
